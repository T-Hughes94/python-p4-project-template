...
if __name__ == '__main__':
    ...
    with app.app_context():
        ...
        # Create some events
        event1 = Event(name="event_1", location="Red Rocks", description="Iron Maiden plays Red Rocks", user_id=user1.id)
        event2 = Event(name="event_2", location="Mishawaka", description="The Bosstones rock the Mish", user_id=user2.id)

        db.session.add_all([event1,event2])
        db.session.commit()

        # Create some food truck events
        food_truck_event1 = FoodTruckEvent(food_sales= 7500.00, beverage_sales=1000.00, food_cost=3000.00, beverage_cost=750.00, fuel_cost=100.00, food_truck_id=food_truck1.id, hourly_wages=100.00, event_id=event1.id)
        food_truck_event2 = FoodTruckEvent(food_sales=5000.00, beverage_sales=500.00, food_cost=2000.00, beverage_cost=250.00, fuel_cost=50.00, food_truck_id=food_truck2.id, hourly_wages=100.00, event_id=event2.id)
